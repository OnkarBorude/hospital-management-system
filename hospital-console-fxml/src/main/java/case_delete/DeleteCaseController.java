...
public class DeleteCaseController {
	...
	public void deleteButtonClicked() throws IOException, InterruptedException {
		...
		String url=EndPoints.DELETE_CASE+cId.getText();
		DeleteCaseResponse deleteCaseResponse=RestUtil.deleteRequest(url, DeleteCaseResponse.class);
		if(deleteCaseResponse.getResponseCode().equals("0000")) {
			responseMsg.setText("Case Delete Successfully");
			responseMsg.setTextFill(Color.GREEN);
		}
		else {
			responseMsg.setText("Case Deletion Failed");
			responseMsg.setTextFill(Color.RED);
		}
